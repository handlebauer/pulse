 ...
 */
async function loadStationsData(): Promise<RadioStation[]> {
    try {
 ...
 */
async function saveClassifiedData(stations: RadioStation[]): Promise<void> {
    try {
        await fs.writeFile(
            OUTPUT_FILE,
...
async function main() {
    try {
        ...
        let allStations = await loadStationsData()
        ...
        // Process stations in batches to avoid hitting API rate limits
        // and to keep prompts at a manageable size
        const BATCH_SIZE = 20
        const totalBatches = Math.ceil(allStations.length / BATCH_SIZE)

        const classifiedStations: RadioStation[] = [...allStations]
        ...
        for (let i = 0; i < totalBatches; i++) {
            ...
            if (classifications.length > 0) {
                ...
                await saveClassifiedData(classifiedStations)
                console.log(
